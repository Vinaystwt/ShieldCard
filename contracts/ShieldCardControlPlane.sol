...
contract ShieldCardControlPlane is Ownable {
    ...
    // Wave 5: admin rotation and scoped disclosure
    event AdminRotated(address indexed previousAdmin, address indexed newAdmin);
    event AuditorRotated(address indexed previousAuditor, address indexed newAuditor);
    event AdminAccessRegranted(uint256 indexed requestId);
    event DisclosureGranted(uint256 indexed requestId, address indexed auditor);
    // Wave 5: within-budget attestation
    event DeptBudgetAttested(uint8 indexed deptId, uint256 timestamp);
    event PackBudgetAttested(uint8 indexed packId, uint256 timestamp);
    ...
    // Wave 5: scoped disclosure auditor
    ...
    // Wave 5: within-budget attestation handles (ebool result of FHE.lte(used, cap))
    mapping(uint8 => ebool) internal _deptWithinBudget;
    mapping(uint8 => ebool) internal _packWithinBudget;
    mapping(uint8 => uint256) public deptAttestedAt;
    mapping(uint8 => uint256) public packAttestedAt;
    ...
    // =========================================================================
    // Admin / Auditor rotation — Wave 5 (additive)
    // =========================================================================

    /**
     * @notice Rotate the admin storage variable. Caller must be the contract owner.
     *         IMPORTANT: existing FHE.allow grants on previously stored handles
     *         continue to point at the old admin until grantAdminAccess(requestIds)
     *         is called for each handle whose access must follow the rotation.
     *         New handles created after this call grant access to the new admin.
     */
    function setAdmin(address newAdmin) external onlyOwner {
        require(newAdmin != address(0), "admin=0");
        address prev = admin;
        admin = newAdmin;
        emit AdminRotated(prev, newAdmin);
    }

    /**
     * @notice Re-grant the current admin scoped read on a batch of request handles
     *         (encAmount and encStatus). Use after setAdmin() to ensure the new
     *         admin can decrypt prior requests.
     */
    function grantAdminAccess(uint256[] calldata requestIds) external onlyOwner {
        for (uint256 i = 0; i < requestIds.length; i++) {
            uint256 rid = requestIds[i];
            if (rid >= _requests.length) revert RequestNotFound(rid);
            PaymentRequest storage req = _requests[rid];
            FHE.allow(req.encAmount, admin);
            FHE.allow(req.encStatus, admin);
            emit AdminAccessRegranted(rid);
        }
    }

    /**
     * @notice Set the auditor address. The auditor is a scoped-disclosure role —
     *         once set, the admin may call grantAuditorAccess(requestIds) to expose
     *         specific request handles for decryption. The auditor cannot read any
     *         handle they have not been explicitly granted.
     */
    function setAuditor(address newAuditor) external onlyOwner {
        address prev = auditor;
        auditor = newAuditor;
        emit AuditorRotated(prev, newAuditor);
    }

    /**
     * @notice Grant the auditor scoped read access to encAmount and encStatus
     *         for the given request IDs. Only the contract owner may grant.
     *         Auditor must be set first.
     */
    function grantAuditorAccess(uint256[] calldata requestIds) external onlyOwner {
        require(auditor != address(0), "auditor=0");
        for (uint256 i = 0; i < requestIds.length; i++) {
            uint256 rid = requestIds[i];
            if (rid >= _requests.length) revert RequestNotFound(rid);
            PaymentRequest storage req = _requests[rid];
            FHE.allow(req.encAmount, auditor);
            FHE.allow(req.encStatus, auditor);
            emit DisclosureGranted(rid, auditor);
        }
    }

    // =========================================================================
    // Within-budget attestation — Wave 5 (additive)
    // =========================================================================

    /**
     * @notice Compute and store the ebool attestation `FHE.lte(deptUsed, deptCap)`
     *         for a department's current encrypted budget state. Reveals only the
     *         boolean result via decryption — neither the amount nor the cap is
     *         disclosed. Result is granted to {this, admin, auditor (if set)}.
     */
    function attestDeptWithinBudget(uint8 deptId) external onlyOwner {
        if (!deptExists[deptId]) revert DeptNotFound(deptId);
        Department storage dept = _departments[deptId];
        require(dept.budgetSet, "dept budget unset");

        ebool result = FHE.lte(dept.encUsedBudget, dept.encBudgetCap);
        FHE.allowThis(result);
        FHE.allow(result, admin);
        if (auditor != address(0)) FHE.allow(result, auditor);

        _deptWithinBudget[deptId] = result;
        deptAttestedAt[deptId]    = block.timestamp;
        emit DeptBudgetAttested(deptId, block.timestamp);
    }

    /**
     * @notice Compute and store `FHE.lte(packUsed, packBudgetLimit)` attestation.
     */
    function attestPackWithinBudget(uint8 packId) external onlyOwner {
        if (!packExists[packId]) revert PackNotFound(packId);
        PolicyPack storage pack = _packs[packId];
        if (!pack.limitsSet) revert PackLimitsNotSet(packId);

        ebool result = FHE.lte(pack.encUsedBudget, pack.encBudgetLimit);
        FHE.allowThis(result);
        FHE.allow(result, admin);
        if (auditor != address(0)) FHE.allow(result, auditor);

        _packWithinBudget[packId] = result;
        packAttestedAt[packId]    = block.timestamp;
        emit PackBudgetAttested(packId, block.timestamp);
    }

    function getDeptWithinBudget(uint8 deptId) external view returns (ebool) {
        return _deptWithinBudget[deptId];
    }

    function getPackWithinBudget(uint8 packId) external view returns (ebool) {
        return _packWithinBudget[packId];
    }
